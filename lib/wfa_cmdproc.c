 ...
 *  input:        pcmdStr -- a string pointer to the command string
 */
int xcCmdProcGetVersion(char* pcmdStr, char* aBuf, int* aLen)
{
 ...
 *  input:        pcmdStr -- a string pointer to the command string
 */
int xcCmdProcAgentConfig(char* pcmdStr, char* aBuf, int* aLen)
{
    char* str;
    int i = 0, j = 0, kwcnt = 0;
    ...
    printProfile(pf);
    hdr->separator = WFA_TLV_SEPARATOR;
    hdr->tag =  WFA_TRAFFIC_AGENT_CONFIG_TLV;
    hdr->len = sizeof(tgProfile_t);

    memcpy(aBuf + WFA_TLV_HDR_LEN, pf, sizeof(tgpf));

    *aLen = WFA_TLV_HDR_LEN + sizeof(tgProfile_t);
 ...
 */
int xcCmdProcAgentSend(char* pcmdStr, char* aBuf, int* aLen)
{
    wfaTLV* hdr = (wfaTLV*) aBuf;
    char* str, *sid;
    int strid;
    ...
    while(1) {
        ...
        memcpy(aBuf + WFA_TLV_HDR_LEN * id_cnt, (char*) &strid, 4);
    }
 ...
 * return - WFA_SUCCESS or WFA_FAILURE;
 */
int xcCmdProcAgentReset(char* pcmdStr, char* aBuf, int* aLen)
{
    wfaTLV* hdr = (wfaTLV*) aBuf;
 ...
 * return - WFA_SUCCESS or WFA_FAILURE;
 */
int xcCmdProcAgentRecvStart(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    while(1) {
        ...
        strid = atoi(sid);
        memcpy(aBuf + WFA_TLV_HDR_LEN + (id_cnt * sizeof(uint32_t)), &strid,  sizeof(uint32_t));
        id_cnt++;
    }
 ...
 * return - WFA_SUCCESS or WFA_FAILURE;
 */
int xcCmdProcAgentRecvStop(char* pcmdStr, char* aBuf, int* aLen)
{
    wfaTLV* hdr = (wfaTLV*) aBuf;
    char* str, *sid;
    int strid;
...
int xcCmdProcAgentSendPing(char* pcmdStr, char* aBuf, int* aLen)
{
    wfaTLV* hdr = (wfaTLV*) aBuf;
    ...
    *aLen = hdr->len + WFA_TLV_HDR_LEN;

    return WFA_SUCCESS;
}

int xcCmdProcAgentStopPing(char* pcmdStr, char* aBuf, int* aLen)
{
    wfaTLV* hdr = (wfaTLV*) aBuf;
    char* str;
    int strid;
    ...
    memcpy(aBuf + WFA_TLV_HDR_LEN, (char*) &strid, 4);

    hdr->separator = WFA_TLV_SEPARATOR;
    hdr->tag =  WFA_TRAFFIC_STOP_PING_TLV;
    hdr->len = 4;  /* multiple 4s if more streams */

    *aLen = WFA_TLV_HDR_LEN + 4;

    return WFA_SUCCESS;
}

int xcCmdProcStaGetIpConfig(char* pcmdStr, char* aBuf, int* aLen)
{
    int slen;
    char* str = NULL;
    ...
    wfaEncodeTLV(WFA_STA_GET_IP_CONFIG_TLV, WFA_IF_NAME_LEN, (char*) &getipconf, aBuf);

    *aLen = WFA_TLV_HDR_LEN + WFA_IF_NAME_LEN;

    return WFA_SUCCESS;
}

int xcCmdProcStaSetIpConfig(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_IP_CONFIG_TLV, sizeof(staSetIpConfig), (char*) &staSetIpConfig, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(staSetIpConfig);

    return WFA_SUCCESS;
}

int xcCmdProcStaGetMacAddress(char* pcmdStr, char* aBuf, int* aLen)
{
    int slen;
    char* str = NULL;
    dutCommand_t getmac;
    ...
    wfaEncodeTLV(WFA_STA_GET_MAC_ADDRESS_TLV, WFA_IF_NAME_LEN, (char*) &getmac, aBuf);
...
int xcCmdProcStaSetMacAddress(char* pcmdStr, char* aBuf, int* aLen)
{
    char* str = NULL;
    dutCommand_t setmac;
    ...
    wfaEncodeTLV(WFA_STA_SET_MAC_ADDRESS_TLV, WFA_IF_NAME_LEN + 18, (char*) &setmac, aBuf);

    *aLen = WFA_TLV_HDR_LEN + WFA_IF_NAME_LEN + 18;

    return WFA_SUCCESS;
}

int xcCmdProcStaIsConnected(char* pcmdStr, char* aBuf, int* aLen)
{
    int slen;
    char* str = NULL;
    ...
    wfaEncodeTLV(WFA_STA_IS_CONNECTED_TLV, WFA_IF_NAME_LEN, (char*) &isconnected, aBuf);
...
int xcCmdProcStaVerifyIpConnection(char* pcmdStr, char* aBuf, int* aLen)
{
    wfaTLV* hdr = (wfaTLV*) aBuf;
    ...
    wfaEncodeTLV(WFA_STA_VERIFY_IP_CONNECTION_TLV, WFA_IF_NAME_LEN + sizeof(caStaVerifyIpConnect_t), (char*) &verifyip, aBuf);

    hdr->separator = WFA_TLV_SEPARATOR;
    hdr->tag =  WFA_STA_VERIFY_IP_CONNECTION_TLV;
    hdr->len = WFA_IF_NAME_LEN + sizeof(caStaVerifyIpConnect_t);

    *aLen = WFA_TLV_HDR_LEN + WFA_IF_NAME_LEN + sizeof(caStaVerifyIpConnect_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaGetBSSID(char* pcmdStr, char* aBuf, int* aLen)
{
    char* str = NULL;
    ...
    wfaEncodeTLV(WFA_STA_GET_BSSID_TLV, WFA_IF_NAME_LEN, (char*) &getbssid, aBuf);
...
int xcCmdProcStaGetStats(char* pcmdStr, char* aBuf, int* aLen)
{
    char* str = NULL;
    ...
    wfaEncodeTLV(WFA_STA_GET_STATS_TLV, WFA_IF_NAME_LEN, (char*) &getstats, aBuf);
...
int  xcCmdProcStaSetEncryption(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_ENCRYPTION_TLV, sizeof(caStaSetEncryption_t), (char*) setencryp, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetEncryption_t);
...
int xcCmdProcStaSetPSK(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    for(;;) {
        ...
        if(strcasecmp(str, "interface") == 0) {
            ...
            str = strtok_r(NULL, ",", &pcmdStr);
//             if(strcmp(str, "wpa") == 0) {
//                 printf("dont support wpa mode, change to mixed\n");
//                 str = "wpa2-wpa-psk";
//             }
...
#endif
    wfaEncodeTLV(WFA_STA_SET_PSK_TLV, sizeof(caStaSetPSK_t), (char*) setencryp, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetPSK_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaSetEapTLS(char* pcmdStr, char* aBuf, int* aLen)
{
...
#endif
    wfaEncodeTLV(WFA_STA_SET_EAPTLS_TLV, sizeof(caStaSetEapTLS_t), (char*) setsec, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetEapTLS_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaSetEapTTLS(char* pcmdStr, char* aBuf, int* aLen)
{
...
#endif
    wfaEncodeTLV(WFA_STA_SET_EAPTTLS_TLV, sizeof(caStaSetEapTTLS_t), (char*) setsec, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetEapTTLS_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaSetEapSIM(char* pcmdStr, char* aBuf, int* aLen)
{
...
#endif
    wfaEncodeTLV(WFA_STA_SET_EAPSIM_TLV, sizeof(caStaSetEapSIM_t), (char*) setsec, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetEapSIM_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaSetPEAP(char* pcmdStr, char* aBuf, int* aLen)
{
...
#endif
    wfaEncodeTLV(WFA_STA_SET_PEAP_TLV, sizeof(caStaSetEapPEAP_t), (char*) setsec, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetEapPEAP_t);

    return WFA_SUCCESS;
}


int xcCmdProcStaSetIBSS(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_IBSS_TLV, sizeof(caStaSetIBSS_t), (char*) setibss, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetIBSS_t);

    return WFA_SUCCESS;
}

int xcCmdProcDeviceGetInfo(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_DEVICE_GET_INFO_TLV, WFA_IF_NAME_LEN + sizeof(caDevInfo_t), (char*) dutCmd, aBuf);

    *aLen = WFA_TLV_HDR_LEN + WFA_IF_NAME_LEN + sizeof(caDevInfo_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaGetInfo(char* pcmdStr, char* aBuf, int* aLen)
{
    char* str;
    ...
    wfaEncodeTLV(WFA_STA_GET_INFO_TLV, WFA_IF_NAME_LEN, (char*) getInfo, aBuf);
...
int xcCmdProcStaUpload(char* pcmdStr, char* aBuf, int* aLen)
{
    char* str;
    dutCommand_t* dutCmd = (dutCommand_t*)(aBuf + sizeof(wfaTLV));
    ...
    wfaEncodeTLV(WFA_STA_UPLOAD_TLV, WFA_IF_NAME_LEN + sizeof(caStaUpload_t), (char*) dutCmd, aBuf);

    *aLen = WFA_TLV_HDR_LEN + WFA_IF_NAME_LEN + sizeof(caStaUpload_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaAssociate(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_ASSOCIATE_TLV, WFA_IF_NAME_LEN + sizeof(caStaAssociate_t), (char*) setassoc, aBuf);

    *aLen = WFA_TLV_HDR_LEN + WFA_IF_NAME_LEN + sizeof(caStaAssociate_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaReAssociate(char* pcmdStr, char* aBuf, int* aLen)
{
    dutCommand_t* setassoc = (dutCommand_t*)(aBuf + sizeof(wfaTLV));
    char* str;
    caStaAssociate_t* assoc = &setassoc->cmdsu.assoc;
    ...
    wfaEncodeTLV(WFA_STA_REASSOCIATE_TLV, WFA_IF_NAME_LEN + sizeof(caStaAssociate_t), (char*) setassoc, aBuf);
...
int xcCmdProcDeviceListIF(char* pcmdStr, char* aBuf, int* aLen)
{
    printf("xcCmdProcDeviceListIf\n");
    char tmp[sizeof(dutCommand_t)] = {0};
    dutCommand_t* getdevlist = (dutCommand_t*) tmp;
    char* str;
    ...
    wfaEncodeTLV(WFA_DEVICE_LIST_IF_TLV, sizeof(dutCommand_t), tmp, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(dutCommand_t);
...
int xcCmdProcStaSetUAPSD(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    memcpy(aBuf + WFA_TLV_HDR_LEN, setuapsd, sizeof(caStaSetUAPSD_t));

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetUAPSD_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaDebugSet(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_DEBUG_SET_TLV, WFA_IF_NAME_LEN + sizeof(staDebugSet_t), (char*) debugSet, aBuf);

    *aLen = WFA_TLV_HDR_LEN + WFA_IF_NAME_LEN + sizeof(staDebugSet_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaSetMode(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_MODE_TLV, sizeof(caStaSetMode_t), (char*) setmode, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetMode_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaGetP2pDevAddress(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_GET_DEV_ADDRESS_TLV, WFA_IF_NAME_LEN, (char*) getP2pDevAdd, aBuf);
...
int xcCmdProcStaSetP2p(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_SETP2P_TLV, sizeof(caStaSetP2p_t), (char*) staSetP2p, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetP2p_t);

    return true;
}


int xcCmdProcStaP2pConnect(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_CONNECT_TLV, sizeof(caStaP2pConnect_t), (char*) staP2pConnect, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaP2pConnect_t);
...
int xcCmdProcStaP2pStartGroupFormation(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_START_GRP_FORMATION_TLV, sizeof(caStaP2pStartGrpForm_t), (char*) staP2pStartGrpForm, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaP2pStartGrpForm_t);

    return true;
}

int xcCmdProcStaP2pDissolve(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_DISSOLVE_TLV, sizeof(caStaP2pDissolve_t), (char*) staP2pDissolve, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaP2pDissolve_t);

    return true;
}

int xcCmdProcStaSendP2pInvReq(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_SEND_INV_REQ_TLV, sizeof(caStaSendP2pInvReq_t), (char*) staSendP2pInvReq, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSendP2pInvReq_t);

    return true;
}

int xcCmdProcStaAcceptP2pInvReq(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_ACCEPT_INV_REQ_TLV, sizeof(caStaAcceptP2pInvReq_t), (char*) staAccceptP2pInvReq, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaAcceptP2pInvReq_t);
...
int xcCmdProcStaSendP2pProvDisReq(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_SEND_PROV_DIS_REQ_TLV, sizeof(caStaSendP2pProvDisReq_t), (char*) staSendP2pProvDisReq, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSendP2pProvDisReq_t);

    return true;
}

int xcCmdProcStaSetWpsPbc(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_WPS_SETWPS_PBC_TLV, sizeof(caStaSetWpsPbc_t), (char*) staSetWpsPbc, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetWpsPbc_t);

    return true;
}

int xcCmdProcStaWpsReadPin(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_WPS_READ_PIN_TLV, sizeof(caStaWpsReadPin_t), (char*) staWpsReadPin, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaWpsReadPin_t);

    return true;
}


int xcCmdProcStaWpsReadLabel(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_WPS_READ_LABEL_TLV, sizeof(caStaWpsReadLabel_t), (char*) staWpsReadLabel, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaWpsReadLabel_t);

    return true;
}


int xcCmdProcStaWpsEnterPin(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_WPS_ENTER_PIN_TLV, sizeof(caStaWpsEnterPin_t), (char*) staWpsEnterPin, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaWpsEnterPin_t);

    return true;
}

int xcCmdProcStaGetPsk(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_GET_PSK_TLV, sizeof(caStaGetPsk_t), (char*) staGetPsk, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaGetPsk_t);

    return true;
}

int xcCmdProcStaP2pStartAutoGo(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_START_AUTO_GO_TLV, sizeof(caStaStartAutoGo_t), (char*) staP2pStartAutoGo, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaStartAutoGo_t);

    return true;
}


int xcCmdProcStaP2pReset(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_RESET_TLV, WFA_IF_NAME_LEN, (char*) staP2pReset, aBuf);
...
int xcCmdProcStaGetP2pIpConfig(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_GET_IP_CONFIG_TLV, sizeof(caStaGetP2pIpConfig_t), (char*) staGetP2pIpConfig, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaGetP2pIpConfig_t);

    return true;
}

int xcCmdProcStaSendServiceDiscoveryReq(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_SEND_SERVICE_DISCOVERY_REQ_TLV, sizeof(caStaSendServiceDiscoveryReq_t), (char*) staSendServiceDiscoveryReq, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSendServiceDiscoveryReq_t);

    return true;
}

int xcCmdProcStaSendP2pPresenceReq(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_SEND_PRESENCE_REQ_TLV, sizeof(caStaSendP2pPresenceReq_t), (char*) staSendP2pPresenceReq, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSendP2pPresenceReq_t);

    return true;
}


int xcCmdProcStaSetSleepReq(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_SET_SLEEP_TLV, sizeof(caStaSetSleep_t), (char*) staSetSleep, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetSleep_t);

    return true;
}


int xcCmdProcStaSetOpportunistcPsReq(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_SET_OPPORTUNISTIC_PS_TLV, sizeof(caStaSetOpprPs_t), (char*) staSetOpprPs, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetOpprPs_t);

    return true;
}
int xcCmdProcStaAddARPTableEntry(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_ADD_ARP_TABLE_ENTRY_TLV, sizeof(caStaAddARPTableEntry_t), (char*) staAddARPTableEntry, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaAddARPTableEntry_t);

    return true;
}
int xcCmdProcStaBlockICMPResponse(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_P2P_BLOCK_ICMP_RESPONSE_TLV, sizeof(caStaBlockICMPResponse_t), (char*) staBlockICMPResponse, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaBlockICMPResponse_t);
...
int xcCmdProcStaSetPwrSave(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_PWRSAVE_TLV, sizeof(caStaSetPwrSave_t), (char*) setps, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetPwrSave_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaSetWMM(char* pcmdStr, char* aBuf, int* aLen)
{
#ifdef WFA_WMM_AC
 ...
 Fixed %d, MSDU Size: %d, Max MSDU Size %d, MIN SERVICE INTERVAL: %d, MAX SERVICE INTERVAL: %d\
    ...
    memcpy(aBuf + WFA_TLV_HDR_LEN, setwmm, sizeof(caStaSetWMM_t));

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetWMM_t);
#endif
    return WFA_SUCCESS;
}

int xcCmdProcStaSetEapFAST(char* pcmdStr, char* aBuf, int* aLen)
{
...
#endif
    wfaEncodeTLV(WFA_STA_SET_EAPFAST_TLV, sizeof(caStaSetEapFAST_t), (char*) setsec, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetEapFAST_t);

    return WFA_SUCCESS;
}


int xcCmdProcStaSetEapAKA(char* pcmdStr, char* aBuf, int* aLen)
{
...
#endif
    wfaEncodeTLV(WFA_STA_SET_EAPAKA_TLV, sizeof(caStaSetEapAKA_t), (char*) setsec, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetEapAKA_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaSetSystime(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_SYSTIME_TLV, sizeof(caStaSetSystime_t), (char*) systime, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetSystime_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaDisconnect(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_DISCONNECT_TLV, WFA_IF_NAME_LEN, (char*) disc, aBuf);
...
int xcCmdProcStaSendNeigReq(char* pcmdStr, char* aBuf, int* aLen)
{
    char* str;
    dutCommand_t* getInfo = (dutCommand_t*)(aBuf + sizeof(wfaTLV));
    ...
    wfaEncodeTLV(WFA_STA_SEND_NEIGREQ_TLV, WFA_IF_NAME_LEN, (char*) getInfo, aBuf);
...
int xcCmdProcStaDevSendFrame(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_DEV_SEND_FRAME_TLV, WFA_IF_NAME_LEN + sizeof(caStaDevSendFrame_t), (char*) cmd, aBuf);

    *aLen = WFA_TLV_HDR_LEN + WFA_IF_NAME_LEN + sizeof(caStaDevSendFrame_t);

    return WFA_SUCCESS;
}


int xcCmdProcStaTestBedCmd(char* pcmdStr, char* aBuf, int* aLen)
{
    dutCommand_t* info = (dutCommand_t*)(aBuf + sizeof(wfaTLV));

    DPRINT_INFO(WFA_OUT, "This is a TestBed Station Command ONLY\n");

    wfaEncodeTLV(WFA_STA_SEND_NEIGREQ_TLV, WFA_IF_NAME_LEN, (char*) info, aBuf);
...
//#ifdef WFA_STA_TB
int xcCmdProcStaPresetTestParameters(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_PRESET_PARAMETERS_TLV, sizeof(caStaPresetParameters_t), (char*) presetTestParams, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaPresetParameters_t);
...
int xcCmdProcStaResetDefault(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_RESET_DEFAULT_TLV, sizeof(caStaResetDefault_t), (char*) reset, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaResetDefault_t);
...
int xcCmdProcStaSetRadio(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_RADIO_TLV, WFA_IF_NAME_LEN + sizeof(caStaSetRadio_t), (char*) cmd, aBuf);
    *aLen = WFA_TLV_HDR_LEN + WFA_IF_NAME_LEN + sizeof(caStaSetRadio_t);
    return WFA_SUCCESS;
}

/* If you decide to use CLI, the function is to be disabled */

int xcCmdProcStaSetWireless(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_WIRELESS_TLV, sizeof(caStaSetWireless_t), (char*) staWirelessParams, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetWireless_t);
    return WFA_SUCCESS;
}

int xcCmdProcStaSendADDBA(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SEND_ADDBA_TLV, sizeof(caStaSetSendADDBA_t), (char*) staSendADDBA, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetSendADDBA_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaSetRIFS(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_RIFS_TEST_TLV, sizeof(caStaSetRIFS_t), (char*) staSetRIFS, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSetRIFS_t);
...
int xcCmdProcStaSendCoExistMGMT(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SEND_COEXIST_MGMT_TLV, sizeof(caStaSendCoExistMGMT_t), (char*) staSendMGMT, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaSendCoExistMGMT_t);

    return WFA_SUCCESS;
}


int xcCmdProcStaSet11n(char* pcmdStr, char* aBuf, int* aLen)
{
    caSta11n_t* v11nParams = (caSta11n_t*)(aBuf + sizeof(wfaTLV));
    char* str;
    caSta11n_t init11nParams = {"wlan0", 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFFFF, "", "", 0xFF, 0xFF, 0xFF, 0xFF};
    ...
    wfaEncodeTLV(WFA_STA_SET_11N_TLV, sizeof(caSta11n_t), (char*) v11nParams, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caSta11n_t);
    return WFA_SUCCESS;
}
...
int xcCmdProcStaSetRFeature(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_RFEATURE_TLV, sizeof(caStaRFeat_t), (char*) rfeat, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaRFeat_t);
...
int xcCmdProcStaStartWfdConnection(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_START_WFD_CONNECTION_TLV, sizeof(caStaStartWfdConn_t), (char*) staStartWfdConn, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaStartWfdConn_t);
    return WFA_SUCCESS;
}

int xcCmdProcStaCliCommand(char* pcmdStr, char* aBuf, int* aLen)
{

    printf("\n The CA CLI command to DUT is : %s", pcmdStr);
    printf("\n The CA CLI command to DUT Length : %zu", strlen(pcmdStr));
    wfaEncodeTLV(WFA_STA_CLI_CMD_TLV, strlen(pcmdStr), (char*) pcmdStr, aBuf);

    *aLen = WFA_TLV_HDR_LEN + strlen(pcmdStr);
    return true;
...
int xcCmdProcStaConnectGoStartWfd(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_CONNECT_GO_START_WFD_TLV, sizeof(caStaConnectGoStartWfd_t), (char*) staConnectGoStartWfd, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaConnectGoStartWfd_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaGenerateEvent(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_GENERATE_EVENT_TLV, sizeof(caStaGenEvent_t), (char*) staGenEvent, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaGenEvent_t);

    return WFA_SUCCESS;
}


int xcCmdProcStaReinvokeWfdSession(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_REINVOKE_WFD_SESSION_TLV, sizeof(caStaReinvokeWfdSession_t), (char*) staReinvokeWfdSession, aBuf);
    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaReinvokeWfdSession_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaGetParameter(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_GET_PARAMETER_TLV, sizeof(caStaGetParameter_t), (char*) staGetParameter, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaGetParameter_t);

    return WFA_SUCCESS;
}


int xcCmdProcStaNfcAction(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_NFC_ACTION_TLV, sizeof(caStaNfcAction_t), (char*) staNfcAction, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaNfcAction_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaExecAction(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_EXEC_ACTION_TLV, sizeof(caStaExecAction_t), (char*) staExecAction, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaExecAction_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaInvokeCommand(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_INVOKE_COMMAND_TLV, sizeof(caStaInvokeCmd_t), (char*) staInvokeCmd, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaInvokeCmd_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaManageService(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_MANAGE_SERVICE_TLV, sizeof(caStaMngServ_t), (char*) staManageServCmd, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaMngServ_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaGetEvents(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_GET_EVENTS_TLV, sizeof(caStaGetEvents_t), (char*) staGetEvents, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaGetEvents_t);

    return WFA_SUCCESS;
}

int xcCmdProcStaGetEventDetails(char* pcmdStr, char* aBuf, int* aLen)
{
    ...
    wfaEncodeTLV(WFA_STA_GET_EVENT_DETAILS_TLV, sizeof(caStaGetEventDetails_t), (char*) staGetEventDetails, aBuf);

    *aLen = WFA_TLV_HDR_LEN + sizeof(caStaGetEventDetails_t);
