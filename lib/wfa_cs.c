...
int agtCmdProcGetVersion(int len, char* parms, int* respLen, char* respBuf)
{
    ...
    strncpy(getverResp->cmdru.version, WFA_SYSTEM_VER, WFA_VERNAM_LEN);

    wfaEncodeTLV(WFA_GET_VERSION_RESP_TLV, 8 + WFA_VERSION_LEN, (char*) getverResp, respBuf);

    *respLen = WFA_TLV_HDR_LEN + 8 + WFA_VERSION_LEN;
...
int wfaStaAssociate(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    (void) ssid;
    char res[255] = {0};
    FILE* fd = popen("uci get -q wireless.station.disabled", "r");

    if(fd != NULL) {
        fgets(res, sizeof(res), fd);
    ...
    if(strchr(res, '1') == NULL) {
        ...
        */
        ...
        // stop the supplicant
        system("uci set -q wireless.station.disabled=0");
        system("uci set -q wireless.apconfig.disabled=1");
        system("uci commit -q wireless");
        system("wifi");
    }

    staAssocResp->status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_ASSOCIATE_RESP_TLV, 4, (char*) staAssocResp, respBuf);
 ...
 *    with the AP.
 */
int wfaStaReAssociate(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    DPRINT_INFO(WFA_OUT, "entering wfaStaAssociate ...\n");

    return wfaStaAssociate(len, caCmdBuf, respLen, respBuf);
}
...
int wfaStaIsConnected(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    if(tmpfile == NULL) {
        ...
        wfaEncodeTLV(WFA_STA_IS_CONNECTED_RESP_TLV, 4, (char*) staConnectResp, respBuf);
        *respLen = WFA_TLV_HDR_LEN + 4;
    ...
    wfaEncodeTLV(WFA_STA_IS_CONNECTED_RESP_TLV, 12, (char*) staConnectResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 12;
...
int wfaStaGetIpConfig(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
     ...
     */
    ret = access("/usr/local/sbin/getipconfig.sh", X_OK);

    if(ret == -1) {
        ipconfigResp->status = STATUS_ERROR;
        wfaEncodeTLV(WFA_STA_GET_IP_CONFIG_RESP_TLV, 4, (char*) ipconfigResp, respBuf);
        *respLen = WFA_TLV_HDR_LEN + 4;
    ...
    for(;;) {
        if(fgets(string, 256, tmpfd) == NULL || strlen(string) == 0) {
            break;
        }
    ...
    wfaEncodeTLV(WFA_STA_GET_IP_CONFIG_RESP_TLV, 8 + sizeof(caStaGetIpConfigResp_t), (char*) ipconfigResp, respBuf);

    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaGetIpConfigResp_t);
...
int wfaStaSetIpConfig(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_IP_CONFIG_RESP_TLV, 4, (char*) staSetIpResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaVerifyIpConnection(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    if(tmpfile == NULL) {
        ...
        wfaEncodeTLV(WFA_STA_VERIFY_IP_CONNECTION_RESP_TLV, 4, (char*) verifyIpResp, respBuf);
        *respLen = WFA_TLV_HDR_LEN + 4;
    ...
    wfaEncodeTLV(WFA_STA_VERIFY_IP_CONNECTION_RESP_TLV, 12, (char*) verifyIpResp, respBuf);
...
int wfaStaGetMacAddress(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    if(tmpfd == NULL) {
        ...
        wfaEncodeTLV(WFA_STA_GET_MAC_ADDRESS_RESP_TLV, 4, (char*) getmacResp, respBuf);
        *respLen = WFA_TLV_HDR_LEN + 4;
    ...
    wfaEncodeTLV(WFA_STA_GET_MAC_ADDRESS_RESP_TLV, 4 + WFA_MAC_ADDR_STR_LEN, (char*) getmacResp, respBuf);

    *respLen = WFA_TLV_HDR_LEN + 4 + WFA_MAC_ADDR_STR_LEN;
...
int wfaStaGetStats(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_GET_STATS_RESP_TLV, 4, (char*) statsResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaSetEncryption1(int len, char *caCmdBuf, int *respLen, char *respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_ENCRYPTION_RESP_TLV, 4, (char *)setEncrypResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaSetEncryption(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    dutCmdResponse_t* setEncrypResp = &gGenericResp;
    int i;
    sprintf(gCmdStr, "uci set -q wireless.station.ssid=\"%s\"", setEncryp->ssid);
    sret = system(gCmdStr);

    if(setEncryp->encpType == ENCRYPT_NONE) {
        printf("setting network to open\n");
        system("uci set -q wireless.station.encryption=none");
        system("uci delete -q wireless.station.key");
    }
    else if(setEncryp->encpType == ENCRYPT_WEP) {

        system("uci set -q wireless.station.encryption=wep");

        for(i = 0; i < 4; i++) {
            if(setEncryp->keys[i][0] != '\0') {
                sprintf(gCmdStr, "uci set -q wireless.station.key%i=\"%s\"", i + 1, setEncryp->keys[i]);
                sret = system(gCmdStr);
            }
        }
        ...
        if(setEncryp->keys[i][0] != '\0') {
            sprintf(gCmdStr, "uci set -q wireless.station.key=%d", setEncryp->activeKeyIdx + 1);
            sret = system(gCmdStr);
        }
    }

    sret = system("uci set -q wireless.station.disabled=0");
    sret = system("uci set -q wireless.apconfig.disabled=1");
    system("uci commit -q wireless");
    system("wifi");
    sleep(10);
...
int wfaStaSetEapTLS(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    sret = system("uci set -q wireless.station.disabled=0");
    sret = system("uci set -q wireless.apconfig.disabled=1");
    sprintf(gCmdStr, "uci set -q wireless.station.ssid=\"%s\"", setTLS->ssid);
    sret = system(gCmdStr);
    system("uci set -q wireless.station.encryption=none");
    system("uci delete -q wireless.station.psk");

    sret = system(gCmdStr);
    sret = system("uci commit -q wireless");
    sret = system("wifi");
    ...
    if(strcasecmp(setTLS->keyMgmtType, "wpa2-sha256") == 0) {
        ...
        sprintf(gCmdStr, "wpa_cli -i %s set_network 0 key_mgmt WPA2-EAP", ifname);
    }
    ...
    wfaEncodeTLV(WFA_STA_SET_EAPTLS_RESP_TLV, 4, (char*) setEapTlsResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetPSK(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    sret = system(gCmdStr);
#else
    printf("set %s netowrk: \"%s\"'", setPSK->intf, setPSK->ssid);

    sprintf(gCmdStr, "uci set -q wireless.station.ssid=\"%s\"", setPSK->ssid);
    sret = system(gCmdStr);

    if(setPSK->encpType != ENCRYPT_NONE) {
        printf("encryption:%d\n", setPSK->encpType);
        sprintf(gCmdStr, "uci set -q wireless.station.key=\"%s\"", setPSK->passphrase);
        sret = system(gCmdStr);

        char cypher[12] = {0};

        switch(setPSK->encpType) {
            case ENCRYPT_TKIP:
                sprintf(cypher, "tkip");
                break;

            case ENCRYPT_AESCCMP_TKIP:
                sprintf(cypher, "tkip+aes");
                break;

            case ENCRYPT_AESCCMP:
            default:
                sprintf(cypher, "aes");
                break;
        }
        ...
        if(strcasecmp(setPSK->keyMgmtType, "wpa2-sha256") == 0) {
            ...
            sprintf(gCmdStr, "uci set -q wireless.station.encryption=\"psk2+%s\"", cypher);
            sret = system(gCmdStr);
        }
        ...
        else if(strcasecmp(setPSK->keyMgmtType, "wpa2-wpa-psk") == 0 || strcasecmp(setPSK->keyMgmtType, "wpa") == 0) {
            sprintf(gCmdStr, "uci set -q wireless.station.encryption=\"psk-mixed+%s\"", cypher);
            sret = system(gCmdStr);
        }
        else {
    ...
    wfaEncodeTLV(WFA_STA_SET_PSK_RESP_TLV, 4, (char*) setPskResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaGetInfo(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
     ...
     */
    sprintf(infoResp.cmdru.info, "interface,%s,vendor,Ralink,cardtype,802.11n", getInfo->intf);

    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_GET_INFO_RESP_TLV, 8 + WFA_INFO_BUFSIZE, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + WFA_INFO_BUFSIZE;
...
//   sprintf(cmdStr, "wpa_cli -i %s set_network 0 pairwise '\"%s\"", ifname, setTTLS->encrptype);
    ...
    wfaEncodeTLV(WFA_STA_SET_EAPTTLS_RESP_TLV, 4, (char*) setEapTtlsResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetEapSIM(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_EAPSIM_RESP_TLV, 4, (char*) setEapSimResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetPEAP(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_PEAP_RESP_TLV, 4, (char*) setPeapResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetUAPSD(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_UAPSD_RESP_TLV, 4, (char*) setUAPSDResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
    return WFA_SUCCESS;
}
...
int wfaDeviceGetInfo(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    caDeviceGetInfoResp_t dinfo = {"Twist Home", "Twist Speaker", WFA_SYSTEM_VER};
    ...
    infoResp->status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_DEVICE_GET_INFO_RESP_TLV, 8 + sizeof(caDeviceGetInfoResp_t), (char*) infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caDeviceGetInfoResp_t);
...
int wfaDeviceListIF(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    DPRINT_INFO(WFA_OUT, "IFace Type: %d\n", ifList->cmdsu.iftype);
    ...
    switch(ifList->cmdsu.iftype) {
        ...
        default: {
            ...
            wfaEncodeTLV(WFA_DEVICE_LIST_IF_RESP_TLV, 8 + sizeof(caDeviceListIFResp_t), (char*) infoResp, respBuf);
            *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caDeviceListIFResp_t);
...
int wfaStaDebugSet(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_GET_INFO_RESP_TLV, 4, (char*) debugResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaGetBSSID(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    if(tmpfd == NULL) {
        ...
        wfaEncodeTLV(WFA_STA_GET_BSSID_RESP_TLV, 4, (char*) bssidResp, respBuf);
        *respLen = WFA_TLV_HDR_LEN + 4;
    ...
    wfaEncodeTLV(WFA_STA_GET_BSSID_RESP_TLV, 8 + WFA_MAC_ADDR_STR_LEN, (char*) bssidResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + WFA_MAC_ADDR_STR_LEN;
...
int wfaStaSetIBSS(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_IBSS_RESP_TLV, 4, (char*) setIbssResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
 ...
 *    7. active  key
 */
int wfaStaSetMode(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_MODE_RESP_TLV, 4, (char*) SetModeResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;

    return WFA_SUCCESS;
}

int wfaStaSetPwrSave(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_PWRSAVE_RESP_TLV, 4, (char*) SetPSResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;

    return WFA_SUCCESS;
}

int wfaStaUpload(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    if(upload->type == WFA_UPLOAD_VHSO_RPT) {
        ...
        if(e2efp == NULL) {
            ...
            wfaEncodeTLV(WFA_STA_UPLOAD_RESP_TLV, 4, (char*) upLoadResp, respBuf);
            *respLen = WFA_TLV_HDR_LEN + 4;
            return WFA_FAILURE;
        }
        ...
        wfaEncodeTLV(WFA_STA_UPLOAD_RESP_TLV, 8 + sizeof(caStaUploadResp_t), (char*) upLoadResp, respBuf);
        *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaUploadResp_t);
    }
    else {
        upLoadResp->status = STATUS_ERROR;
        wfaEncodeTLV(WFA_STA_UPLOAD_RESP_TLV, 4, (char*) upLoadResp, respBuf);
 ...
 */
int wfaStaSetWMM(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
#ifdef WFA_WMM_AC
    ...
    wfaEncodeTLV(WFA_STA_SET_WMM_RESP_TLV, 4, (char*) setwmmResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
#endif
...
int wfaStaSendNeigReq(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SEND_NEIGREQ_RESP_TLV, 4, (char*) sendNeigReqResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetEapFAST(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_EAPFAST_RESP_TLV, 4, (char*) setEapFastResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetEapAKA(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_EAPAKA_RESP_TLV, 4, (char*) setEapAkaResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetSystime(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_SYSTIME_RESP_TLV, 4, (char*) setSystimeResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaPresetParams(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    if(presetParams->supplicant == eWpaSupplicant) {
        FILE* fd = NULL;
        fd = popen("pgrep wpa_supplicant", "r");
    ...
    wfaEncodeTLV(WFA_STA_PRESET_PARAMETERS_RESP_TLV, 4, (char*) PresetParamsResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSet11n(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_11N_RESP_TLV, 4, (char*) v11nParamsResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
    return WFA_SUCCESS;
}
int wfaStaSetWireless(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_WIRELESS_RESP_TLV, 4, (char*) staWirelessResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
    return WFA_SUCCESS;
}
...
int wfaStaSendADDBA(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_SEND_ADDBA_RESP_TLV, 4, (char*) staSendADDBAResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
    return WFA_SUCCESS;
}
...
int wfaStaSetRIFS(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_RIFS_TEST_RESP_TLV, 4, (char*) staSetRIFSResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSendCoExistMGMT(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SEND_COEXIST_MGMT_RESP_TLV, 4, (char*) staSendMGMTResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaResetDefault(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_RESET_DEFAULT_RESP_TLV, 4, (char*) ResetResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaTestBedCmd(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_DISCONNECT_RESP_TLV, 4, (char*) staCmdResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaDevSendFrame(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_DEV_SEND_FRAME_RESP_TLV, 4, (char*) devSendResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetMacAddr(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_MAC_ADDRESS_RESP_TLV, 4, (char*) staCmdResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaDisconnect(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    // stop the supplicant
    system("uci set -q wireless.station.disabled=1");
    system("uci set -q wireless.apconfig.disabled=1");
    system("uci commit -q wireless");
    system("wifi");
    ...
    wfaEncodeTLV(WFA_STA_DISCONNECT_RESP_TLV, 4, (char*) staDiscResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
//    char *addr = staPing->dipaddr;
    ...
    if(staPing->iptype == 2) {
        if(tos > 0)
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;/usr/local/sbin/wfaping6.sh %s %ss -c %i -Q %d -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
                    streamid, streamid, bflag, staPing->dipaddr, totalpkts, tos,  staPing->frameSize, streamid);
        else
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;/usr/local/sbin/wfaping6.sh %s %s -c %i -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
        ...
        if(tos > 0)
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;/usr/local/sbin/wfaping.sh %s %s -c %i  -Q %d -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
                    streamid, streamid, bflag, staPing->dipaddr, totalpkts, tos, staPing->frameSize, streamid);
        else
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;/usr/local/sbin/wfaping.sh %s %s -c %i -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
                    streamid, streamid, bflag, staPing->dipaddr, totalpkts, staPing->frameSize, streamid);
    ...
    int sleeptime = staPing->duration + 6;
...
int wfaStaGetP2pDevAddress(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_GET_DEV_ADDRESS_RESP_TLV, 8 + WFA_P2P_DEVID_LEN, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + WFA_P2P_DEVID_LEN;
...
int wfaStaSetP2p(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_SETP2P_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaP2pConnect(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_CONNECT_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaStartAutoGo(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_START_AUTO_GO_RESP_TLV, 8 + WFA_P2P_GRP_ID_LEN, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + WFA_P2P_GRP_ID_LEN;
...
int wfaStaP2pStartGrpFormation(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_START_GRP_FORMATION_RESP_TLV, 8 + sizeof(caP2pStartGrpFormResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caP2pStartGrpFormResp_t);
...
int wfaStaP2pDissolve(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_DISSOLVE_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSendP2pInvReq(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_SEND_INV_REQ_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaAcceptP2pInvReq(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_ACCEPT_INV_REQ_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSendP2pProvDisReq(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_SEND_PROV_DIS_REQ_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetWpsPbc(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_WPS_SETWPS_PBC_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaWpsReadPin(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_WPS_READ_PIN_RESP_TLV, 8 + WFA_WPS_PIN_LEN, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + WFA_WPS_PIN_LEN;
...
int wfaStaWpsEnterPin(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_WPS_ENTER_PIN_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaGetPsk(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_GET_PSK_RESP_TLV, 8 + sizeof(caP2pStaGetPskResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caP2pStaGetPskResp_t);
...
int wfaStaP2pReset(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_RESET_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaGetP2pIpConfig(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_GET_IP_CONFIG_RESP_TLV, 8 + sizeof(caStaGetIpConfigResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaGetIpConfigResp_t);
...
int wfaStaSendServiceDiscoveryReq(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_SEND_SERVICE_DISCOVERY_REQ_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSendP2pPresenceReq(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_SEND_PRESENCE_REQ_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetSleepReq(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_SET_SLEEP_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetOpportunisticPsReq(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_SET_OPPORTUNISTIC_PS_RESP_TLV, sizeof(infoResp), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaPresetParams(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_PRESET_PARAMETERS_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSet11n(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    if(v11nParams->addba_reject != 0xFF && v11nParams->addba_reject < 2) {
        ...
        if(st != 0) {
            ...
            wfaEncodeTLV(WFA_STA_SET_11N_RESP_TLV, 8 + WFA_INFO_BUFSIZE, (char*) v11nParamsResp, respBuf);
            *respLen = WFA_TLV_HDR_LEN + 8 + WFA_INFO_BUFSIZE;
            return FALSE;
        }
    }
    ...
    if(v11nParams->amsdu != 0xFF && v11nParams->amsdu < 2) {
        systemf("uci set wireless.radio0.amsdu=%d", v11nParams->amsdu);
    ...
    if(v11nParams->greenfield != 0xFF && v11nParams->greenfield < 2) {
        systemf("uci set wireless.radio0.greenfield=%d", v11nParams->greenfield);
    ...
    if(v11nParams->sgi20 != 0xFF && v11nParams->sgi20 < 2) {
        systemf("uci set wireless.radio0.short_gi_20=%d", v11nParams->sgi20);
    ...
    if(v11nParams->stbc_rx != 0xFF) {
        systemf("uci set wireless.radio0.rx_stbc=%d", v11nParams->stbc_rx);
    ...
    if(v11nParams->stbc_tx != 0xFF) {
        systemf("uci set wireless.radio0.tx_stbc=%d", v11nParams->stbc_tx);

        if(st != 0) {
            v11nParamsResp->status = STATUS_ERROR;
            strcpy(v11nParamsResp->cmdru.info, "set_stbc_tx failed");
            wfaEncodeTLV(WFA_STA_SET_11N_RESP_TLV, 8 + WFA_INFO_BUFSIZE, (char*) v11nParamsResp, respBuf);
            *respLen = WFA_TLV_HDR_LEN + 8 + WFA_INFO_BUFSIZE;
            return FALSE;
        }
...
int wfaStaAddArpTableEntry(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_ADD_ARP_TABLE_ENTRY_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaBlockICMPResponse(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_P2P_BLOCK_ICMP_RESPONSE_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetRadio(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_RADIO_RESP_TLV, 4, (char*) staCmdResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaSetRFeature(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    wfaEncodeTLV(WFA_STA_SET_RFEATURE_RESP_TLV, 4, (char*) caResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaStartWfdConnection(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_START_WFD_CONNECTION_RESP_TLV, 8 + sizeof(caStaStartWfdConnResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaStartWfdConnResp_t);
...
cleanup:
    ...
    wfaEncodeTLV(WFA_STA_CLI_CMD_RESP_TLV, sizeof(caStaCliCmdResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + sizeof(caStaCliCmdResp_t);
...
//  caStaConnectGoStartWfd_t *staConnecGoStartWfd= (caStaConnectGoStartWfd_t *)caCmdBuf; //uncomment and use it
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_CONNECT_GO_START_WFD_RESP_TLV, 8 + sizeof(caStaStartWfdConnResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaStartWfdConnResp_t);
...
int wfaStaGenerateEvent(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_GENERATE_EVENT_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
//  caStaReinvokeWfdSession_t *staReinvokeSession= (caStaReinvokeWfdSession_t *)caCmdBuf; //uncomment and use it
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_REINVOKE_WFD_SESSION_RESP_TLV, 4, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 4;
...
int wfaStaGetParameter(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    if(staGetParam->program == PROG_TYPE_WFDS) {
        ...
        if(staGetParam->getParamValue == eOpenPorts) {
            ...
            // Get all the open ports and make space seperated list and return, che124ck list is not bigger than 128 bytes.
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_GET_PARAMETER_RESP_TLV, 8 + sizeof(caStaGetParameterResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaGetParameterResp_t);
...
int wfaStaNfcAction(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_NFC_ACTION_RESP_TLV, 8 + sizeof(caStaNfcActionResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaNfcActionResp_t);
...
int wfaStaExecAction(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_EXEC_ACTION_RESP_TLV, 4 + 16, (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + 16;
...
int wfaStaInvokeCommand(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_INVOKE_CMD_RESP_TLV, 8 + sizeof(caStaInvokeCmdResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaInvokeCmdResp_t);
...
int wfaStaManageService(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_MANAGE_SERVICE_RESP_TLV, 8 + sizeof(caStaConnSessCmdResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaConnSessCmdResp_t);
...
int wfaStaGetEvents(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_GET_EVENTS_RESP_TLV, 8 + sizeof(caStaGetEventListCmdResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaGetEventListCmdResp_t);
...
int wfaStaGetEventDetails(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    infoResp.status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_GET_EVENT_DETAILS_RESP_TLV, 8 + sizeof(caStaGetEventDetailsCmdResp_t), (char*) &infoResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + 8 + sizeof(caStaGetEventDetailsCmdResp_t);
