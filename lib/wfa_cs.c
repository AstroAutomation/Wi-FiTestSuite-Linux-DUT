...
int wfaStaGetInfo(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
     ...
     */
    sprintf(infoResp.cmdru.info, "interface,%s,vendor,<Vendor>,cardtype,802.11<a/b/g/n/ac>", getInfo->intf);
...
int wfaDeviceGetInfo(int len, char* caCmdBuf, int* respLen, char* respBuf)
{
    ...
    caDeviceGetInfoResp_t dinfo = {"<Company Name>", "<Product Name>", WFA_SYSTEM_VER};
...
//    char *addr = staPing->dipaddr;
    ...
    if(staPing->iptype == 2) {
        if(tos > 0)
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;/usr/local/sbin/wfaping6.sh %s %s -c %i -Q %d -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
